...
def vllm_basic_inference(model_name: str = "meta-llama/Llama-2-7b-chat-hf") -> str:
    ...
    try:
        ...
        sampling_params = SamplingParams(
            temperature=0.7, top_p=0.9, max_tokens=512, stop=["</s>", "User:"]
        )
...
def compare_quantization_levels():
    ...
    for level, info in comparison.items():
        print(
            f"{level:<10} {info['size']:<20} {info['quality']:<12} {info['speed']:<12} {info['ram_required']:<10}"
        )
...
def batch_processing_with_concurrency(
    prompts: List[str], model: str = "llama3.2", max_workers: int = 4
) -> List[str]:
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(process_single_prompt, p): i for i, p in enumerate(prompts)
        }
