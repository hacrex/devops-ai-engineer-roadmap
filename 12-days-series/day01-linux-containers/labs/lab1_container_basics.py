...
"""
    ...
    build_command = ["docker", "build", "-t", "my-hello-app:1.0", "-f", "Dockerfile.lab2", "."]
...
def exercise_3_volume_mounting():
    ...
    try:
        ...
        result = subprocess.run(read_command, capture_output=True, text=True, timeout=30)
...
"""
    ...
    run_command = [
        ...
        "--name", container_name,
        "-p", "8080:80",
        "-v", f"{os.getcwd()}/html:/usr/share/nginx/html",
        "nginx:alpine"
    ]
...
def exercise_5_networking():
    ...
    try:
        ...
        result = subprocess.run(create_command, capture_output=True, text=True, timeout=30)
        ...
        if result.returncode == 0:
            ...
            subprocess.run(["docker", "rm", "-f", "web"], capture_output=True, timeout=10)
            web_command = [
            ...
            subprocess.run(["docker", "rm", "-f", "api"], capture_output=True, timeout=10)
            api_command = [
            ...
            ping_result = subprocess.run(ping_command, capture_output=True, text=True, timeout=30)
            ...
            if ping_result.returncode == 0:
                ...
                inspect_result = subprocess.run(inspect_command, capture_output=True, text=True, timeout=30)
